...
### Added
- Full-screen Bubble Tea TUI behind the `tui` feature flag, replacing the survey-based interactive UI (in progress). Completed screens: main menu with output viewer (8.0–8.1), source selection (8.2), live-filtered service selection with async catalog loading and registry sync status polling (8.3), and registry trust warning with metadata display and explicit confirmation (8.4).
